#!/usr/bin/env python3
"""
Verification tests for the backtesting implementation.

This script verifies:
1. Reproducibility with fixed seed
2. Performance degradation with worsening costs/slippage
3. Sanity baseline check with zero signal strategy
"""
...
def create_random_signal_strategy(prices: pd.Series) -> pd.Series:
    """
    Create a random signal strategy.

    Args:
        prices: Price series

    Returns:
        pandas Series with random signals between -1 and 1
    """
    # Set random seed for reproducibility
    np.random.seed(42)
    # Create random signals between -1 and 1
    signals = pd.Series(np.random.uniform(-1, 1, len(prices)), index=prices.index)
    print("Created random signal strategy")
    return signals
...
def test_reproducibility(prices: pd.Series, n_runs: int = 3) -> bool:
    ...
    signals = create_random_signal_strategy(prices)
...
def test_sanity_baseline(prices: pd.Series) -> bool:
    ...
    # Check that performance ≈ 0 before costs
    performance_near_zero_no_cost = abs(cagr_no_cost) < 0.0001  # Very close to zero

    # Check that performance < 0 after costs (should be negative due to friction)
    performance_negative_with_cost = cagr_with_cost < 0

    print(f"Performance near zero without costs: {performance_near_zero_no_cost}")
    print(f"Performance negative with costs: {performance_negative_with_cost}")

    if performance_near_zero_no_cost and performance_negative_with_cost:
