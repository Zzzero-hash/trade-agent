...
from trade_agent.agents.sl.config_loader import SLConfig, load_config
...
class HyperparameterTuner:
    ...
    def __init__(self, model_type: str, cv_strategy: TemporalCV,
                 scoring_metric: str = 'neg_mean_squared_error') -> None:
        """
    ...
    def tune(self, X: np.ndarray, y: np.ndarray,
        ...
        for params in param_combinations:
            try:
                ...
                warnings.warn(f"Skipping parameter combination {params} due to error: {e}", stacklevel=2)
                continue
...
class SLTrainingPipeline:
    ...
    def __init__(self, config: dict[str, Any] | SLConfig) -> None:
        """
        ...
        # Use canonical model_settings only (legacy model_config removed)
        self.model_config = getattr(self.config, 'model_settings', {})
    ...
    # Optional: extract data & schema hashes if data_path present
        ...
        if data_path and os.path.exists(data_path):
            try:
                ...
                from trade_agent.data.schema import extract_schema
            ...
            except Exception:  # broad but logged
                pass
...
def train_model_from_config(
    ...
    if target_column not in df.columns:
        raise ValueError(
            f"Target column '{target_column}' not found; first columns: {list(df.columns)[:10]}"
        )
    ...
    with contextlib.suppress(Exception):
        sl_config.data_path = data_path
    pipeline = SLTrainingPipeline(sl_config)
    return pipeline.train(X, y)
