...
class SLConfig(CoreConfig):
    ...
    Backwards compatibility: legacy configs used ``model_config``; we accept
    that key and map it to ``model_settings`` automatically. ``tuning_config``
    is optional in simple tests so defaults to empty dict.
    """
...
def load_config(config_input: str | dict[str, Any]) -> SLConfig:
...
def _load_raw_config(input_data: str | dict[str, Any]) -> dict[str, Any]:
...
def _validate_config(raw: dict[str, Any]) -> SLConfig:
    ...
    if 'model_config' in raw and 'model_settings' not in raw:
        raw['model_settings'] = raw.pop('model_config')
    try:
        return SLConfig(**raw)
