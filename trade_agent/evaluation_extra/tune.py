#!/usr/bin/env python3
"""Deprecated legacy tuner stub.

Original experimental hyperparameter tuning code produced excessive lint/type
noise and is removed from the default distribution for the repository health
milestone (TRA-10). This stub remains so that historical import paths do not
break. Activate the legacy implementation (if still present locally) by
setting the environment variable ``TRADE_AGENT_ENABLE_LEGACY_TUNER=1`` and
placing the old implementation in ``_tune_legacy_impl.py`` next to this file.
"""

from __future__ import annotations

__all__ = ["main", "run_legacy_tuner"]

def run_legacy_tuner(argv: list[str] | None = None) -> int:  # pragma: no cover
    return 0  # intentionally inert stub

def main() -> int:  # pragma: no cover
    return run_legacy_tuner(argv=None)
...
# Import project modules (primary path first, then fallbacks). Keep broad
# try/except but avoid indentation errors that previously broke lint/type checks.
try:  # Primary import path
    from stable_baselines3 import PPO, SAC  # type: ignore
    from trade_agent.eval.backtest import BacktestEngine  # type: ignore
...
except ImportError:  # Fallbacks (legacy paths or missing optional deps)
    try:
        ...
        from trade_agent.agents.sl.train import SLTrainingPipeline  # type: ignore
        from trade_agent.data.splits import purged_walk_forward_splits  # type: ignore
    except ImportError:  # Minimal stubs when nothing available
