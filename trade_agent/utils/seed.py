...
def _seed_env(env: Any, seed: int) -> None:  # noqa: C901 - acceptable complexity for central seeding
    ...
    for space_name in ("action_space", "observation_space"):
        ...
        if space is not None and hasattr(space, "seed"):
            with contextlib.suppress(Exception):  # pragma: no cover - non critical
                space.seed(seed)


def set_seed(  # noqa: C901 - central orchestration acceptable
    seed: int | None = 42,
    *,
    env: Any | None = None,
    ...
    if torch is not None:  # pragma: no branch - simple guard
        with contextlib.suppress(Exception):  # Seed RNGs first
            torch.manual_seed(seed_int)
            if torch.cuda.is_available():  # type: ignore[attr-defined]
                torch.cuda.manual_seed(seed_int)  # type: ignore[attr-defined]
                torch.cuda.manual_seed_all(seed_int)  # type: ignore[attr-defined]
        if deterministic:
            with contextlib.suppress(Exception):  # cuDNN flags
    ...
    if verbose:  # pragma: no cover - logging side effect
        # Verbose flag currently no-op (future: logging integration)
        pass
