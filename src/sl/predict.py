...
class SLPredictionPipeline:
    ...
    def predict_quantiles(self, X: Union[np.ndarray, pd.DataFrame],
        ...
        return dict(zip(quantiles, predictions.T))

    def batch_predict(self, X: Union[np.ndarray, pd.DataFrame],
...
def predict_from_config(config_path: str) -> np.ndarray:
    ...
    # Make predictions
    predictions = predict_from_model(model_path, data_path, output_path, config)
