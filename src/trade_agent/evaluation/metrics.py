...
Pure implementations with shared financial ratios imported from the local
``financial_metrics`` module (no legacy shims retained).
"""
...
def compute_metrics(
    ...
    win_rate = (
        0.0 if active_returns.empty else float((active_returns > 0).mean())
    )

    exposure = float(pos.abs().mean()) if not pos.empty else 0.0
    turnover = (
        float(pos.diff().abs().sum() / (len(pos) - 1)) if len(pos) > 1 else 0.0
    )
