...
def purged_walk_forward_splits(
    ...
    if not data.index.is_monotonic_increasing:
        warnings.warn("Data is not sorted by timestamp. Sorting now.")
    ...
    for i in range(n_splits):
        ...
        if split_end - split_start < 3:
            warnings.warn(f"Split {i} has insufficient data. Skipping.")
            continue
...
def example_usage():
    ...
    print("Example 1: Purged Walk-Forward Splits")
    print("=" * 40)
    ...
    for train, val, test in purged_walk_forward_splits(
        ...
        if len(val) > 0:
            print(f"  Val range: {val.index[0]} to {val.index[-1]}")
        if len(test) > 0:
            print(f"  Test range: {test.index[0]} to {test.index[-1]}")
        print()

    print("Example 2: Strict Temporal Split Generator")
    print("=" * 40)

    # Generate temporal splits
    window_count = 0
    for X_window, y_target, t_index in strict_temporal_split_generator(
        data,
        window_size=10,
        step_size=5
    ):
        ...
        print(f"  y_target index: {t_index[0]}")
        print(f"  y_target value: {y_target.values}")
        range_str = f"  X_window time range: {X_window.index[0]} " \
